...
- [ ] Civitai連携 - モデル/LoRA情報をCivitaiから自動取得
  - **検討日**: 2024-12-13
  - **見送り理由**:
    1. 名前マッチングの精度問題（ファイル名とCivitai登録名の表記揺れ）
    2. ハッシュ未保存（最も正確な`by-hash` APIが使えない）
    3. 限定的なメリット（Civitai未登録リソースには効果なし）
  - **将来実装する場合**:
    1. メタデータパーサーでモデルハッシュを抽出・保存する機能を先に実装
    2. ハッシュベース検索 + 名前検索のハイブリッド方式
    3. ユーザー確認 → DB保存の流れ
  - **詳細**: `.claude/plans/streamed-snuggling-dusk.md` 参照
