...
- [ ] ワークフロー保存・管理機能
  - **背景**: ComfyUIはJSON形式のワークフローで画像を生成するが、ワークフローの管理が煩雑になりがち
  - **機能概要**:
    - 画像のメタデータからワークフローJSONを抽出・保存
    - ワークフローにタグ付け・検索機能
    - 類似ワークフローのグループ化
    - ワークフローのdiff表示（2つのワークフローの差分比較）
  - **工数目安**: 中〜大

- [ ] プロンプトテンプレートライブラリ
  - **背景**: 同じプロンプトフレーズを何度も入力するのは非効率
  - **機能概要**:
    - よく使うプロンプト断片の保存・カテゴリ分類
    - プロンプト断片の組み合わせでテンプレート作成
    - 高評価画像で使われたプロンプトの自動抽出・提案
    - クリップボードへのコピー機能
  - **工数目安**: 中

- [ ] ネガティブプロンプトライブラリ
  - **背景**: ネガティブプロンプトは定型化しやすいが、目的別に使い分けたい
  - **機能概要**:
    - 目的別ネガティブプロンプト集（リアル系、アニメ系、NSFW除去等）
    - 高評価画像のネガティブプロンプト傾向分析
    - ベースモデル別のおすすめネガティブプロンプト
  - **工数目安**: 小〜中

### 比較・分析機能

- [ ] A/B比較ビュー
  - **背景**: パラメータを変えて生成した画像の違いを確認したい
  - **機能概要**:
    - 2枚の画像を左右に並べて表示
    - メタデータの差分をハイライト表示（変更された設定値を強調）
    - スライダーで重ね合わせ比較
    - 比較結果の保存・共有
  - **工数目安**: 中

- [ ] シード変種エクスプローラー
  - **背景**: 同じ設定で複数シードを試した結果をまとめて見たい
  - **機能概要**:
    - 同一プロンプト・設定で異なるシードの画像を自動グループ化
    - シード値による検索・フィルタリング
    - ベストシードの記録・再利用
  - **工数目安**: 中

- [ ] 生成パラメータ最適化分析
  - **背景**: どの設定が高品質な画像を生み出すか知りたい
  - **機能概要**:
    - Steps / CFG Scale / Sampler の組み合わせ別成功率
    - LoRAウェイト値と評価の相関分析
    - モデル×LoRA×サンプラーの相性マトリクス
    - 推奨設定の自動提案
  - **工数目安**: 大

- [ ] 生成実験タイムライン
  - **背景**: パラメータを少しずつ変えて実験した履歴を追跡したい
  - **機能概要**:
    - 時系列でパラメータ変更履歴を可視化
    - 分岐点（大きく設定を変えた箇所）の検出
    - 実験ノートとしてメモ追加機能
...
- [ ] 生成スタイルプリセット
  - **背景**: モデル+LoRA+サンプラー+CFG等の組み合わせを再利用したい
  - **機能概要**:
    - 成功した設定の組み合わせをプリセットとして保存
    - プリセットにサンプル画像を紐付け
...
- [ ] モデル互換性データベース
  - **背景**: LoRAやVAEがどのベースモデルと相性が良いか把握したい
  - **機能概要**:
    - ベースモデル×LoRAの使用実績と評価を集計
    - 相性の良い/悪い組み合わせを可視化
    - 新しいLoRA導入時の推奨ベースモデル提案
  - **工数目安**: 中
