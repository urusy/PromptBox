...
| 選択ツールバー | 選択モード時に常時表示、未選択時は操作ボタン非活性 |
...
| 項目 | 仕様 |
|------|------|
| 画像表示 | オリジナルサイズ表示、ズーム機能（クリックでライトボックス） |
| メタデータ表示 | 主要項目の整形表示 + 生JSON折りたたみ表示 |
| 操作 | ★評価、タグ編集、ダウンロード、削除、プロンプトコピー |
| ナビゲーション | 前後画像への移動（ボタン/キーボード）、一覧への戻り |
| キーボード操作 | `←`/`→`: 前後移動、`0`-`5`: レーティング設定、`Esc`: ライトボックス閉じる |
| レイアウト | レスポンシブ（モバイル: 縦積み、デスクトップ: 横並び） |
| 検索条件維持 | 一覧からの遷移時に検索パラメータを保持、戻り時に再現 |
...
| パスワード | bcryptハッシュ化 |
| セッション有効期限 | 7日間 |
| 未認証時 | ログイン画面へリダイレクト（ログイン後に元のURLへ戻る） |
...
| 起動 | 一覧画面のスライドショーボタン |
| 表示 | フルスクリーンモード |
| 自動再生 | 設定可能な間隔（2〜15秒、デフォルト5秒） |
| 再生順序 | 順次/ランダム切り替え可能 |
| 操作 | 再生/一時停止、前後移動、終了 |
| キーボード | `Space`: 再生/停止、`←`/`→`: 前後移動、`Esc`: 終了 |
| UI | 3秒間操作がないとコントロール自動非表示 |
